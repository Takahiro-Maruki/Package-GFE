...
Program GFE_v2.1.cpp to estimate allele and genotype frequencies 
from pro files of individual high-throughput sequencing data 
of multiple individuals from a population.  In this version, the 
genotype-call (c) mode is added.  An error message is given when 
the identity of the minor allele is ambiguous in this version.  
In this version, the minimum and maximum coverage for each individual 
can be specified by users.  In this version 2.1, users can print out
the information at all sites in the c mode by setting the `-as' option
at one.
		...
		if (pop_coverage > 0) {
			...
			if (pop_read[n1] < pop_coverage) {
				...
				if (den_best_p == 0.0) {
					...
					// Print out the results for the f mode and c mode with the as option at one
					if (s_mode == "f") {
					...
					if (maxll > -10000000000.0) {
						...
						if (best_p < 1.0) {
							...
							// Print out the monomorphism results for the f mode and c mode with the as option equal to one
							if (s_mode == "f") { 
								test_Mac = 2.0*nsample*best_p - (int)(2.0*nsample*best_p);
								if ( test_Mac >= 0.5) {
                                                			best_Mac = (int)(2.0*nsample*best_p) + 1;
						...
						fprintf(stderr, "ML estimates not found at site %d on %s\n", site, scaffold.c_str());
						// Print out the results for the f mode at c mode with the as option equal to one
						if (s_mode == "f") {
							fprintf(outstream, "%s\t%d\t%s\tNA\tNA\t%d\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\n", scaffold.c_str(), site, ref_nuc.c_str(), pop_coverage);
							// printf("%s\t%d\t%s\tNA\tNA\t%d\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\n", scaffold.c_str(), site, ref_nuc.c_str(), pop_coverage);
						} else if (s_mode == "c" && as == 1) {
			...
			// Print out the reference-sequence information when there is no data for the f mode and c mode with the as option equal to one
			if (s_mode == "f") {
